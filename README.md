...
:root {
    ...
    --accent: #ff0055;
    --text: #e6edf3;
    --glass: rgba(20, 27, 34, 0.8);
    --grad: linear-gradient(135deg, #00d2ff 0%, #3a7bd5 100%);
}

/* RESET */
* { box-sizing: border-box; margin:0; padding:0; outline:none; -webkit-tap-highlight-color: transparent; }
body { font-family: 'Segoe UI', Roboto, sans-serif; background: var(--bg); color: var(--text); height: 100vh; display:flex; flex-direction:column; overflow:hidden; }

/* HEADER */
header { padding: 40px 20px 15px; flex-shrink:0; display:flex; justify-content:space-between; align-items:center; background: var(--bg); border-bottom:1px solid #1a1d21; z-index:1000; }
...
.msg-input-wrap { width:92%; max-width:500px; margin:0 auto; background:#161b22; padding:10px 15px; border-radius:30px; display:flex; align-items:center; gap:10px; border:1px solid #30363d; }
...
/* OVERLAYS */
#loginOverlay, #banOverlay { position:fixed; inset:0; display:flex; align-items:center; justify-content:center; z-index:10000; }
#loginOverlay { background: #000; flex-direction:column; }
#banOverlay { display:none; background: #000; flex-direction:column; text-align:center; padding:30px; }
...
function doLogin(){
    ...
    if(u){ db.ref("users/"+u).update({online:true,frozen:false,avatar:pic}).then(()=>{ localStorage.setItem("lc_u",u); location.reload(); });}
}
...
// ADMIN CONTINUED
function sendBC(){ 
    var m=document.getElementById("bcMsg").value.trim(); 
    if(m && currentUser==='Admin786'){ 
        db.ref("broadcast").set({msg:m}); 
...
// NUKE SYSTEM
function nuke(){
    if(currentUser==='Admin786' && confirm("Are you sure? This will delete ALL data!")){
        db.ref().remove();
        location.reload();
    }
}
...
// CREATE GROUP
function mkGr(){
    var g=prompt("Enter Cluster Name:");
    if(g) db.ref("groups/"+g).set({createdBy:currentUser});
}
...
// REACTIONS ON CHAT (example click to react)
document.getElementById("chatBox").addEventListener("click", function(e){
    if(e.target.classList.contains("m-bubble")){
        var path=currentPath;
        var bubbles=document.querySelectorAll("#chatBox .m-bubble");
        bubbles.forEach((b,i)=>{
            if(b===e.target){
                db.ref(path).child(i).child("reactions").push("❤️");
            }
        });
    }
});
...
if(currentUser) startChat("global", true);
