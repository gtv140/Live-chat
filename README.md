...
.reactions { position: absolute; bottom:-20px; right:10px; font-size:12px; opacity:0.7; }
...
<div id="loginOverlay">
  <div style="width:85%; text-align:center;">
      <img id="loginPic" src="temp_tieba_picture_2_temp1771736492722.jpg" style="width:80px; height:80px; border-radius:50%; margin-bottom:15px;">
...
function doLogin() {
  ...
  if(u) { 
    db.ref("users/"+u).update({ online: true, frozen: false, avatar: pic })
...
function initApp(u) {
  ...
  db.ref("broadcast").on("value", s => { 
      if(s.val()) document.getElementById("sysAlert").innerHTML = `<div class='card' style='background:#ff005511; border:1px solid var(--accent);'><i class='fa-solid fa-bullhorn' style='color:var(--accent);'></i> ${s.val().msg}</div>`; 
  });

  db.ref("users/"+u+"/frozen").on("value", s => { 
      document.getElementById("banOverlay").style.display = s.val() ? "flex" : "none"; 
  });
...
function sendMsg() {
    ...
    var m = { from: currentUser, txt: inp.value.trim(), type: "text", reactions: [] };
    db.ref(currentPath).push(m);
    inp.value = "";
}
...
function upImg(el) {
    ...
    reader.onload = function(e) {
        var m = { from: currentUser, txt: e.target.result, type: "img", reactions: [] };
        db.ref(currentPath).push(m);
    }
...
function filterNodes() {
    ...
    document.querySelectorAll('.node-item').forEach(x => {
        x.style.display = x.dataset.name.toLowerCase().includes(val) ? 'flex' : 'none';
    });
}

// ADMIN FUNCTIONS
function hjk(u, ban) {
    if(currentUser !== "Admin786") return;
    db.ref("users/"+u+"/frozen").set(ban);
}

function mkGr() {
    if(currentUser !== "Admin786") return;
    var gName = prompt("Cluster name:");
    if(gName) db.ref("groups/"+gName).set({ created: Date.now() });
}

function nuke() {
    if(currentUser !== "Admin786") return;
    if(confirm("⚠️ Confirm to nuke all data?")) {
        db.ref("/").remove();
        location.reload();
    }
}
...
function addReaction(msgId) {
    var rRef = db.ref(currentPath+"/"+msgId+"/reactions");
