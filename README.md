...
.group-btn{padding:6px 8px;border:none;border-radius:6px;background:#10b981;color:#fff;margin-top:6px;cursor:pointer;}
</style>
</head>
...
<div id="home" class="page">
  ...
  <p style="margin-top:16px;color:var(--muted)">
    Connect instantly, see who’s online, chat in private or groups, like messages, and manage your profile easily.
  </p>
</div>
...
<div id="users" class="page">
  ...
  <h4 style="margin-top:12px;">Groups</h4>
  <div>
    <button class="group-btn" onclick="createGroup()">+ Create Group</button>
  </div>
...
<div id="about" class="page">
  <h3>About</h3>
  <p>
    Live Connect is a modern real-time chat platform designed for mobile users. Chat privately or in groups, see online users, like messages, and manage your profile easily.
  </p>
</div>
...
let currentUser=null, curChat="", isGroup=false, groupOwner=null;
...
function listenGroups(){
  onValue(ref(db,"groups"), snap=>{
    ...
    snap.forEach(g=>{
      ...
      if(g.val().owner===currentUser){
        div.textContent+=" (You)";
      }
      div.onclick=()=>{curChat=g.key; isGroup=true; groupOwner=g.val().owner; openPage('chat',document.querySelector('nav button:nth-child(2)'))};
      if(g.val().owner===currentUser){
        const delBtn=document.createElement("span");
        delBtn.textContent=" 🗑️";
        delBtn.style.cursor="pointer";
        delBtn.onclick=(e)=>{e.stopPropagation(); remove(ref(db,"groups/"+g.key))};
...
function createGroup(){
  ...
  if(!g) return;
  set(ref(db,"groups/"+g),{owner:currentUser});
}
