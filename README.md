...
/* Existing Body Style - No Change */
body{margin:0;font-family:system-ui,-apple-system,Segoe UI,Roboto;background:var(--bg);color:var(--text);transition:.3s; height: 100dvh; display: flex; flex-direction: column;}
body.light{--bg:#f1f1f1;--card:#fff;--text:#111;--muted:#555;}
.app{max-width:480px;margin:auto;flex:1;display:flex;flex-direction:column; width: 100%;}
...
.page{display:none;padding:16px;flex:1;transition:0.3s; overflow-y: auto;}
...
/* Chat Box - Original Style Ke Sath Height Fix */
.chat-box{background:var(--card);border-radius:18px;padding:14px;height:55vh;overflow-y:auto;margin-bottom:12px;box-shadow:0 0 12px var(--primary);}
...
.msg img{width:100%; border-radius:10px; margin-top:5px;} /* New Image Support */
...
<div class="app">
<header>
<h1 id="mainTitle">Live Connect</h1>
...
<div id="adminPanel" style="display:none; margin-top:20px; padding:10px; border:1px dashed var(--primary); border-radius:15px;">
    <h3 style="color:var(--accent)">Admin Dashboard</h3>
    <button onclick="clearAllChats()" style="padding:10px; background:red; color:white; border:none; border-radius:10px; width:100%;">Clear All Messages</button>
</div>
</div>
...
<div class="input-row">
<label for="imgInp"><i class="fa-solid fa-image" style="color:var(--primary); font-size:24px; cursor:pointer;"></i></label>
...
window.login=()=>{
  ...
  update(userRef,{name:uname,online:true});
...
window.openPage=(id,btn)=>{
  ...
  if(id !== 'chat') document.getElementById("mainTitle").textContent = "Live Connect";
};
...
function loadChat(){
  if(!curChat) return;
  document.getElementById("mainTitle").textContent = curChat;
  ...
  onValue(ref(db,path),snap=>{
    ...
    snap.forEach(m=>{
      ...
      let commentsHTML='<div class="comments">'+(d.comments?Object.values(d.comments).map(c=>`<div><b>${c.from}:</b> ${c.text}</div>`).join(''):'')+'</div>';
      let commentInput=`<div class="comment-input"><input placeholder="Comment"/><button onclick="addComment('${path}','${m.key}',this)">💬</button></div>`;
      div.innerHTML=`<b>${d.from}:</b> ${content} ${actions} ${commentsHTML} ${commentInput}`;
...
window.clearAllChats=()=>{ if(confirm("Clear Everything?")) { remove(ref(db,"chats")); remove(ref(db,"groupChats")); }};
</script>
</body>
</html>
